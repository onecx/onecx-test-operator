
include::onecx-test-operator-attributes.adoc[opts=optional]

{docker-registry}[Link to Docker registry]

== Default properties

.{properties-file}
[%collapsible]
====
[source,properties,subs=attributes+]
----
quarkus.http.auth.permission.health.paths=/q/*
quarkus.http.auth.permission.health.policy=permit
quarkus.http.auth.permission.default.paths=/*
quarkus.http.auth.permission.default.policy=authenticated
quarkus.kubernetes-client.trust-certs=true
%prod.quarkus.oidc-client.client-id=${quarkus.application.name}
quarkus.oidc.ui.auth-server-url=${quarkus.oidc.auth-server-url}
quarkus.oidc.ui.client-id=${quarkus.oidc.client-id}
quarkus.oidc.ui.credentials.secret=${quarkus.oidc.credentials.secret}
quarkus.oidc.ui.application-type=web-app
onecx.test.scheduler.timer=24h
----
====

== Extensions
.Extensions List
[%collapsible]
====
include::onecx-test-operator-extensions.adoc[opts=optional]
====

== Helm

{helm-registry}[Link to Helm registry]

.Default values in {helm-file}
[source,yaml]
----
app:
  serviceAccount:
    enabled: true
  name: operator
  image:
    repository: "onecx/onecx-test-operator"
  envCustom:
    - name: KUBERNETES_NAMESPACE
      valueFrom:
        fieldRef:
          fieldPath: metadata.namespace
    - name: ONECX_TEST_SCHEDULER_TIMER
      value: "24h"
    - name: ONECX_TEST_SCHEDULER_SERVICES_0__URL
      value: ""
    - name: ONECX_TEST_SCHEDULER_SERVICES_0__SERVICES
      value: ""
    - name: ONECX_TEST_SCHEDULER_SERVICES_1__URL
      value: ""
    - name: ONECX_TEST_SCHEDULER_SERVICES_1__SERVICES
      value: ""
  operator:
    keycloak:
      client:
        enabled: true
        spec:
          kcConfig:
            defaultClientScopes: [ocx-pm:read]

----
